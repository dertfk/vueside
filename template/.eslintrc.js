module.exports = {
  ...
  rules: {
    ...
    "comma-spacing": [2, { before: false, after: true }], // 禁止在逗号前使用空格，要求在逗号后使用一个或多个空格
    "comma-style": [2, "last"], // 要求逗号放在数组元素、对象属性或变量声明之后，且在同一行
    "constructor-super": 2, // 要求在构造函数中有 super() 的调用
    "curly": [2, "all"], // 强制所有控制语句使用一致的括号风格，代码块使用大括号包裹
    ...
    "indent": [2, 2, { SwitchCase: 1 }], // 强制使用一致的缩进，2个空格缩进，switch 语句中的 case 子句的缩进级别为1
    "jsx-quotes": [2, "prefer-double"], // 要求所有不包含双引号的 JSX 属性值使用双引号
    "key-spacing": [2, { beforeColon: false, afterColon: true }], // 禁止在对象字面量的键和冒号之间存在空格，要求在对象字面量的冒号和值之间存在至少有一个空格
    "keyword-spacing": [2, { before: true, after: true }], // 要求在关键字之前至少有一个空格
    "new-cap": [2, { newIsCap: true, capIsNew: false }], // 要求构造函数首字母大写
    "new-parens": 2, // 要求调用无参构造函数时有圆括号
    "no-array-constructor": 2, // 禁用 Array 构造函数
    "no-caller": 2, // 用 arguments.caller 或 arguments.callee
    ...
    "no-extra-parens": [2, "functions"], // 禁止不必要的括号，只在函数表达式周围禁止不必要的圆括号
    ...
    "no-this-before-super": 2, // 止在构造函数中，在调用 super() 之前使用 this 或 super
    ...
    "one-var": [2, { initialized: "never" }], // 强制函数中的变量要么一起声明要么分开声明，要求每个作用域的初始化的变量有多个变量声明
    "operator-linebreak": [2, "after"], // 强制操作符使用一致的换行符，要求把换行符放在操作符后面
    "padded-blocks": [2, "never"], // 禁止块语句和类的开始或末尾有空行
    "quotes": [2, "double", { avoidEscape: true, allowTemplateLiterals: true }], // 强制使用一致的双引号，允许字符串使用单引号，只要字符串中包含了一个其它引号，否则需要转义，允许字符串使用反勾号
    "semi": [2, "always"], // 要求使用分号代替 ASI (semi)
    ...
    "object-curly-spacing": [2, "always"], // 强制花括号内有空格 (除了 {})，禁止以对象元素开始或结尾的对象的花括号中有空格
